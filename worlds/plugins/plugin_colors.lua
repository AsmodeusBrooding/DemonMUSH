...
local function init_xterm_colors ()
   ...
   -- Alter Aeon bumps a few very dark xterm colors to brighter values to improve
...
-- default_foreground_color and background_color can be Alter Aeon color codes or MUSHclient's raw numeric color values
         ...
         for i=first_i,num_tokens do
            ...
            if i % 2 == 0 then -- color code
               ...
               if code == XTERM_CODE then -- xterm 256 colors
                  ...
                  -- Alter Aeon treats x1...x15 as normal ANSI colors.
...
-- Convert Alter Aeon and short x codes to 3 digit x codes
...
-- Tries to convert ANSI sequences to Alter Aeon color codes
...
function ColoursToANSI (text)
   ...
   if text:find(CODE_PREFIX, nil, true) then
      ...
      text = text:gsub(X_DIGITS_CAPTURE_PATTERN, function(a)
         ...
         -- Alter Aeon treats x1...x15 as normal ANSI codes
